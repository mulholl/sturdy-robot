	...
	class value{
		public:
			...
			value& setRange(T a, T b){
				...
				if (defSpec){
					if (defVal > maxVal){
						throw inpargs::MaxLessThanDefault { };
					}
					if (defVal < minVal){
						throw inpargs::MinGreaterThanDefault { };
					}
				}
			...
			value& setValList(std::vector<T> inpValList){
				...
				if (defSpec){
					for (typename std::vector<T>::iterator it = valList.begin(); it < valList.end(); ++it){
						...
						if (notInList){
							throw inpargs::ListDoesNotContainDefault{ };
						}
					}
				}
			...
			value& defaultValue(T inpDefVal){
				...
				if (valListSpec){
					for (typename std::vector<T>::iterator it = valList.begin(); it < valList.end(); ++it){
						...
						if (notInList){
							throw inpargs::DefaultOutsideRange{ };
						}
					}
				}
				else {
			...
			value& numArgs(unsigned int n_min, unsigned int n_max){
				...
				if (minArgsSpec && maxArgsSpec){
					 ...
					 */
					if (n_max < n_min){
						throw inpargs::MaxArgsLessThanMinArgs{ };
					}
					if (n_min == n_max){
						exactNumSpec = true;
						rangeNumSpec = false;
